...
export const getCourseCriteria = async (req, res) => {
    try {
        ...
        const [criterios] = await pool.query(
            'SELECT COU_NOT_CRITERIA AS nombre, COU_NOT_PERCENT AS porcentaje, ID_COU_NOTES, COS_ID FROM AMS_COURSE_NOTES WHERE PER_ID = ? AND TEA_ID = ? AND COU_ID = ?',
            [per_id, tea_id, cou_id, id_cou_notes]
        );
